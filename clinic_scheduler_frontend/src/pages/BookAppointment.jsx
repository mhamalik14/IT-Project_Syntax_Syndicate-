...
export default function BookAppointment() {
  ...
  // Fetch providers for a clinic (optional endpoint)
  const fetchProvidersForClinic = async (clinicId) => {
    if (!clinicId) return;
    try {
      const res = await api.get(`/clinics/${clinicId}/providers`);
      if (Array.isArray(res.data)) setProviders(res.data);
    } catch (err) {
      console.warn("Failed to fetch providers for clinic", err);
      setProviders([]);
    }
  };

  // Geolocation: demo selects Durban in KZN
  const tryGeolocate = () => {
    if (!navigator.geolocation) return;
    navigator.geolocation.getCurrentPosition(
      () => {
        const nearestId = "kzn-1";
        setNearest(nearestId);
        const clinic = clinics.find((c) => c.id === nearestId);
        if (clinic) {
          setSelectedProvince(clinic.province);
          setSelectedClinic(nearestId);
        }
      },
  ...
  const selectedClinicDetails = clinics.find((c) => c.id === selectedClinic);
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="w-full flex flex-col items-center text-center">
                    <label className="block text-gray-700 font-semibold mb-3 text-lg flex items-center justify-center gap-2">
                      <span className="text-xl">🗺️</span> Choose Province
                    </label>
                    <select
                    ...
                    {errors.province && <div className="text-red-600 mt-2">{errors.province}</div>}
                  </div>

                  {/* Clinic Select (filtered by province) */}
                  <div className="w-full flex flex-col items-center text-center">
                    <label className="block text-gray-700 font-semibold mb-3 text-lg flex items-center justify-center gap-2">
                    ...
                    {loadingClinics ? (
                      ...
                      >
                        <option value="">{selectedProvince ? "-- Select Clinic --" : "Select a province first"}</option>
                        {filteredClinics.map((c) => (
                          <option key={c.id} value={c.id}>
                            {c.name} {nearest === c.id ? "(Nearest)" : ""} - {c.location}
                          </option>
                        ))}
                      </select>
                    )}
                  ...
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <button
                      ...
                      className={`px-8 py-3 rounded-xl font-semibold shadow-lg transition-all duration-300 transform hover:scale-105 ${
                        booking ? "bg-gray-400 cursor-not-allowed" : "bg-blue-500 hover:bg-blue-600 text-white"
                      }`}
              ...
              <div className="service-card" style={{ flex: 1 }}>
                ...
                <div className="space-y-3 text-gray-700">
                  ...
                  <div className="bg-white p-3 rounded-lg shadow-sm">
                    ...
                    <div className="text-gray-800 font-medium">{formData.serviceReason || "-"}</div>
                  </div>
                  <div className="bg-white p-3 rounded-lg shadow-sm">
                    <div className="font-semibold text-gray-600 flex items-center gap-2 mb-1">
                      <span>📅</span> Date
                    </div>
                    <div className="text-gray-800 font-medium">{formData.appointmentDate || "-"}</div>
                  </div>
                  <div className="bg-white p-3 rounded-lg shadow-sm">
                    <div className="font-semibold text-gray-600 flex items-center gap-2 mb-1">
                      <span>⏰</span> Time Slot
